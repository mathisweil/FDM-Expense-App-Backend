...
def get_claims(request):
    if permission == 'EMPLOYEE':
        ...
        if current:
            ...
            claims = Claim.objects.filter(Q(manager_id=request.user_id) & (Q(status='COMPLETED') | Q(status='APPROVED') | Q(status='REJECTED')))
    elif permission == 'FINANCE':
        if current:
            claims = Claim.objects.filter(Q(employee_id=request.user_id) & ~Q(status='APPROVED'))
        else:
            claims= Claim.objects.filter(Q(employee_id=request.user_id) & (Q(status='PROCESSED') | Q(status='REJECTEDF')))
    else:
